...
- 轉完的整理只做三件不動內容的事：檔名太長就改名（再用「壞連結」修連結）、卡片改外觀（連結裡 `摘要` 換 `膠囊`）、太零碎的小模組用併回鈕併回上層。
...
| toggle 的前後鄰居是清單項目（如 Heart Echo 的 EF 夾在 LVIDd、LA dimension 之間） | 併進同一個清單、用「膠囊」外觀：`- [[<頁名>/<主題>\|膠囊]]`（編號清單接續編號）；連續幾個 toggle 一起併 |
