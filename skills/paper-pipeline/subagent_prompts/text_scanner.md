...
此 agent 做一件事：從 PDF 擷取字詞層級文字和座標，產出 `extracted.json`。這是 text lane 的第一步，下游 worker 依賴這些字詞來組裝段落。
...
```bash
python3 skills/hand-written-pipeline/scripts/extract.py \
  "<paper_dir>/shared/source.pdf" \
  "<output_root>/extracted.json"
```

`extract.py` 會產出 `blocks`，但下游不使用。Scanner 只需確保 `words` 和 `artifact_regions` 正確。
...
source.pdf 是 ground truth（用 Read tool 讀取，pages 參數，每次 ≤20 頁）。`extract.py` 的輸出是候選，需要對照 source.pdf 驗證。

對每頁：
1. 讀 `extracted.json` 中該頁的 `words`、`artifact_regions`、`qc_notes`。
2. 如果版面、符號或 page chrome 可疑，用 Read tool 讀 source.pdf 對應頁面比對。版面概覽可輔助參考 page preview。
...
- **`qc_notes`**：品質檢查中發現但無法安全修正的問題。下游 worker 會參考這些 notes 來識別風險區域。
