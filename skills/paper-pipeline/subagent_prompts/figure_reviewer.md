...
- `skills/hand-written-pipeline/scripts/crop_region.py`：從 `source_page` 裁出指定區域，並用 `--hline` / `--vline` 標出 crop boundary（cyan 線）。
- `skills/hand-written-pipeline/scripts/make_image_preview.py`：把 raw image 轉成 bounded preview。
