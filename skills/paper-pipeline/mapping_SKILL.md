...
| `subagent_prompts/mapping_merger.md` | 寫 Python 腳本將所有 `mapping.<key>.json` 套用回論文 HTML 中，生成重點標註視圖。 |
...
Merger 將讀取 `<paper_dir>/mapping/canonical/` 內所有的 `mapping.*.json`，並寫一段 Python 將它們套用到 `<paper_dir>/reassembly/canonical/paper.html`，完成重點標記與 reader panel 注入後，將最終成果存成 `<paper_dir>/mapping/canonical/paper.html`。
