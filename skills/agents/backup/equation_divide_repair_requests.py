...
Groups requests by equation_id (all requests for the same equation go to one
worker), then uses greedy balancing to distribute equation groups evenly by
request count.

Usage:
    python divide_repair_requests.py <repair_requests_merged.json> \
        --workers 3 \
        --output-dir <paper_dir>/equations/repairs/round_01
...
def greedy_assign(
    equation_groups: list[tuple[str, list[dict]]],
    n: int,
) -> list[list[tuple[str, list[dict]]]]:
    """Assign equation groups to workers by greedy load balancing."""
    buckets: list[list[tuple[str, list[dict]]]] = [[] for _ in range(n)]
    loads = [0] * n

    for equation_id, requests in sorted(equation_groups, key=lambda x: -len(x[1])):
        lightest = min(range(n), key=lambda i: loads[i])
        buckets[lightest].append((equation_id, requests))
...
def main() -> int:
    ...
    by_equation: dict[str, list[dict]] = {}
    for req in requests:
        fig_id = req.get("equation_id", "")
        by_equation.setdefault(fig_id, []).append(req)

    equation_groups = list(by_equation.items())
    workers = min(args.workers, len(equation_groups))
    if workers < args.workers:
        print(f"Warning: reduced workers from {args.workers} to {workers} "
              f"(only {len(equation_groups)} equations with requests)", file=sys.stderr)

    buckets = greedy_assign(equation_groups, workers)
    output_dir = Path(args.output_dir)
    assignments = []

    for idx, bucket in enumerate(buckets):
        repair_id = f"repair_{idx + 1:02d}"
        equation_ids = [fig_id for fig_id, _ in bucket]
        worker_requests = []
        ...
        sub_request = {
            "schema_version": data.get("schema_version", "equation_repair.v1"),
    ...
    result = {
        ...
        "total_equations": len(equation_groups),
    }
