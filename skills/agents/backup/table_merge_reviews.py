...
valid table_review.v1 that downstream scripts (check_review_gate.py,
...
Exit codes:
    0  success
    1  integrity error (duplicate table_id, round mismatch)
    2  input error
"""
...
def main() -> int:
    ...
    for review_path in args.reviews:
        ...
        if review.get("schema_version") != "table_review.v1":
            errors.append(f"{review_path}: schema_version is not table_review.v1")
        ...
        for fig in review.get("tables", []):
            table_id = fig.get("table_id", "")
            if table_id in seen_table_ids:
                errors.append(f"duplicate table_id '{table_id}' across input reviews")
            seen_table_ids.add(table_id)
            all_tables.append(fig)
    ...
    all_tables.sort(key=lambda f: f.get("table_id", ""))

    pass_count = sum(1 for f in all_tables if f.get("decision") == "pass")
    fail_count = sum(1 for f in all_tables if f.get("decision") == "fail")
    finding_count = sum(len(f.get("findings", [])) for f in all_tables)
