...
Groups requests by figure_id (all requests for the same figure go to one
worker), then uses greedy balancing to distribute figure groups evenly by
request count.

Usage:
    python divide_repair_requests.py <repair_requests_merged.json> \
        --workers 3 \
        --output-dir <paper_dir>/figures/repairs/round_01
...
def greedy_assign(
    figure_groups: list[tuple[str, list[dict]]],
    n: int,
) -> list[list[tuple[str, list[dict]]]]:
    """Assign figure groups to workers by greedy load balancing."""
    buckets: list[list[tuple[str, list[dict]]]] = [[] for _ in range(n)]
    loads = [0] * n

    for figure_id, requests in sorted(figure_groups, key=lambda x: -len(x[1])):
        lightest = min(range(n), key=lambda i: loads[i])
        buckets[lightest].append((figure_id, requests))
...
def main() -> int:
    ...
    for req in requests:
        fig_id = req.get("figure_id", "")
        by_figure.setdefault(fig_id, []).append(req)

    figure_groups = list(by_figure.items())
    workers = min(args.workers, len(figure_groups))
    if workers < args.workers:
        print(f"Warning: reduced workers from {args.workers} to {workers} "
              f"(only {len(figure_groups)} figures with requests)", file=sys.stderr)

    buckets = greedy_assign(figure_groups, workers)
    output_dir = Path(args.output_dir)
    assignments = []

    for idx, bucket in enumerate(buckets):
        repair_id = f"repair_{idx + 1:02d}"
        figure_ids = [fig_id for fig_id, _ in bucket]
        worker_requests = []
        ...
        sub_request = {
            "schema_version": data.get("schema_version", "figure_repair.v3"),
