...
此流程使用場景與目標：
- 輸入：一個 pdf 檔案、pages 範圍、worker scope / assignment，以及需要時可呼叫的 render / preview helper
...
15. 視覺檢查後撰寫 `figures.json`。`figures.json` 可以記錄 `verification.result = "fail"` 的 figure；但只要任何 figure 是 fail，`figures.json.status` 就必須是 `incomplete`，不得宣稱此次 figure extraction 成功。

16. `figures.json` 應列出目前確認的圖表成果，包含：
   - figure label
   - `crop_units[]` 中的最終裁切圖片路徑、對應頁碼與裁切框座標
   - 圖說文字或圖說來源
   - 對應的預覽圖與邊界預覽
   - 對應的 `figure_decisions.json` 決策項目
   - 視覺驗證結果

17. 寫出 `figures.json` 前，先做 mechanical self-check。self-check 通過不代表視覺驗證通過；它只確認 manifest、檔案與座標沒有機械錯誤。

18. 若指定範圍中沒有有標記 figure，仍要寫出四個 JSON，`figures` 為空，並在 `figures.json.status` 設為 `complete`。

19. 回報本次寫出的 JSON、讀取過的預覽圖片、產生的 figure crops、通過/失敗的 figures，以及尚未解決的阻礙。不回報 validator command / result，除非 mechanical validator 已經定案。
...
- `expected_panels` 只能根據圖中實際可見的 panel label 或明確視覺結構填寫。不要因為 caption、cross-reference 或正文提到 `Panels A-D` 就自動發明 panel。
  ...
  "figures": [
    {
      ...
      "evidence_read": {
        "final_crop_previews": ["lanes/figures/worker_output/worker_01/previews/Figure_1_preview.png"],
        "edge_previews": [
          "lanes/figures/worker_output/worker_01/previews/Figure_1_top_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_1_bottom_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_1_left_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_1_right_preview.png"
        ]
      },
      "verification": {
    ...
    },
    {
      ...
      "evidence_read": {
        "final_crop_previews": [
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_1_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_2_preview.png"
        ],
        "edge_previews": [
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_1_top_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_1_bottom_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_1_left_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_1_right_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_2_top_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_2_bottom_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_2_left_preview.png",
          "lanes/figures/worker_output/worker_01/previews/Figure_2_part_2_right_preview.png"
        ]
      },
      "verification": {
        "source_context_checked": "pass",
...
- `figure_decisions.json` 和 `figures.json` 的 `crop_units` 必須一致；如果兩者不同，回到 `figure_decisions.json` 修正並重生 final manifest，不要直接手改 `figures.json`。
...
寫出 `figures.json` 前，先做 mechanical self-check。self-check 至少確認：
...
- figure 層不出現可由 `crop_units` 推得的彙總欄位，例如 `pages`、`image_files`、`crop_count`。
