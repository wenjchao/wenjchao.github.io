...
Usage:
    python build_repair_request.py \
        ...
        --canonical-figures <canonical/figures.json> \
        --output <output_path>

--canonical-figures is needed to resolve null crop_id on multi-crop figures.
If omitted, null crop_id will block unless the figure has only one
reviewed_crop_unit.
...
def build_repair_request(
    ...
    canonical_figures: dict | None = None,
) -> dict:
    ...
    # Build crop_unit count lookup from canonical figures.json
    canonical_crop_counts: dict[str, list[str]] = {}
    if canonical_figures:
        for fig in canonical_figures.get("figures", []):
    ...
    for fig in visual_review.get("figures", []):
        if fig.get("decision") != "fail":
            continue

        figure_id = fig.get("figure_id")
        if not figure_id:
            return {"status": "blocked", "reason": "figure missing figure_id"}
        ...
        for finding in fig.get("findings", []):
            ...
            if not finding_id:
                return {"status": "blocked",
                        "reason": f"finding in {figure_id} missing finding_id"}
            ...
            if crop_id:
                ...
                # Try canonical figures.json first
                canonical_crops = canonical_crop_counts.get(figure_id, [])
...
def main() -> int:
    ...
    parser.add_argument("--canonical-figures",
                        help="Path to canonical figures.json (for null crop_id resolution)")
    ...
    canonical_figures = None
    if args.canonical_figures:
        try:
            with open(args.canonical_figures, "r", encoding="utf-8") as f:
                canonical_figures = json.load(f)
        except (json.JSONDecodeError, OSError) as exc:
            print(json.dumps({"status": "error",
                              "reason": f"cannot read canonical figures: {exc}"},
                             indent=2), file=sys.stderr)
            return 2

    outcome = build_repair_request(
        review, args.repair_round, args.repair_id, canonical_figures
    )
