...
Each sub-packet gets a disjoint, consecutive subset of equations and is a valid
equation_review_packet.v1.  The assignment manifest is printed to stdout.

Usage:
    python divide_review_packet.py <review_packet.json> \
        --workers 3 \
        --output-dir <paper_dir>/equations/reviewers/round_00
...
def divide(equations: list[dict], n: int) -> list[list[dict]]:
    per_worker = math.ceil(len(equations) / n)
    return [equations[i : i + per_worker] for i in range(0, len(equations), per_worker)]
...
def main() -> int:
    ...
    if not equations:
        print(json.dumps({"status": "error", "reason": "no equations in packet"}, indent=2),
              file=sys.stderr)
        return 2

    workers = min(args.workers, len(equations))
    if workers < args.workers:
        print(f"Warning: reduced workers from {args.workers} to {workers} "
              f"(only {len(equations)} equations)", file=sys.stderr)

    groups = divide(equations, workers)
    ...
    for idx, group in enumerate(groups):
        ...
        sub_packet = {
            "schema_version": packet.get("schema_version", "equation_review_packet.v1"),
        ...
        assignments.append({
            "reviewer_id": reviewer_id,
            "equation_ids": [fig.get("equation_id") for fig in group],
            "packet_path": str(packet_path),
            "equation_count": len(group),
        })
