...
Repair agent provides intents (figure_id, crop_id, field, new_value). This tool
reads canonical figures.json and figure_decisions.json, looks up the current
...
Usage:
    ...
    echo '[{"figure_id": "F1", "crop_id": "F1", "field": "crop_px", "new_value": [0,0,100,100]}]' \
...
def find_crop_unit(
    data: dict, figure_id: str, crop_id: str
) -> dict | None:
    fig = next(
        (f for f in data.get("figures", []) if f.get("figure_id") == figure_id),
        None,
    )
    if fig is None:
        return None
...
def build_patches(
    ...
    for intent in intents:
        figure_id = intent.get("figure_id")
        crop_id = intent.get("crop_id")
        field = intent.get("field")
        new_value = intent.get("new_value")

        if not figure_id or not crop_id or not field:
            errors.append(
                f"incomplete intent: figure_id={figure_id}, "
        ...
        if field not in ALLOWED_FIELDS:
            errors.append(
                f"{figure_id}/{crop_id}: field '{field}' not in allowed list "
                f"({', '.join(sorted(ALLOWED_FIELDS))})"
            )
            continue

        for manifest_file, data in manifests.items():
            cu = find_crop_unit(data, figure_id, crop_id)
            if cu is None:
                errors.append(
                    f"{figure_id}/{crop_id} not found in {manifest_file}"
                )
                continue
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Path to canonical directory containing figures.json and "
