...
merge_repair.py and validate_equation_repair.py expect.
...
def find_crop_unit_in_manifest(data: dict, equation_id: str, crop_id: str) -> dict | None:
    for fig in data.get("equations", []):
        if fig.get("equation_id") != equation_id:
            continue
...
def build_file_copies(
    crop_id: str, equation_id: str, files: dict[str, list[str]]
) -> list[dict]:
    copies = []
...
def main() -> int:
    ...
    equations_json = manifests["equations.json"]
    ...
    for req in assigned_requests:
        ...
        equation_id = req.get("equation_id", "")
        equation_number = req.get("equation_number", equation_id)
        ...
        canonical_cu = find_crop_unit_in_manifest(
            equations_json, equation_id, crop_id
        )
        if canonical_cu is None:
            errors.append(f"{equation_id}/{crop_id} not found in canonical equations.json")
            repairs.append({
                "request_id": req_id,
                "equation_id": equation_id,
                "equation_number": equation_number,
        ...
        role = canonical_cu.get("role", "complete equation")
        ...
        if crop_id not in processed_crop_ids:
            all_file_copies.extend(build_file_copies(crop_id, equation_id, files))
