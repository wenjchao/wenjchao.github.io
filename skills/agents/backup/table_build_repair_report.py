...
merge_repair.py and validate_table_repair.py expect.
...
def find_crop_unit_in_manifest(data: dict, table_id: str, crop_id: str) -> dict | None:
    for fig in data.get("tables", []):
        if fig.get("table_id") != table_id:
            continue
...
def build_file_copies(
    crop_id: str, table_id: str, files: dict[str, list[str]]
) -> list[dict]:
    copies = []
...
def main() -> int:
    ...
    tables_json = manifests["tables.json"]
    ...
    for req in assigned_requests:
        ...
        table_label = req.get("table_label", table_id)
        ...
        canonical_cu = find_crop_unit_in_manifest(
            tables_json, table_id, crop_id
        )
        if canonical_cu is None:
            errors.append(f"{table_id}/{crop_id} not found in canonical tables.json")
            repairs.append({
                "request_id": req_id,
                "table_id": table_id,
                "table_label": table_label,
        ...
        role = canonical_cu.get("role", "complete table")
        ...
        if crop_id not in processed_crop_ids:
            all_file_copies.extend(build_file_copies(crop_id, table_id, files))
