...
Exit codes:
    0  all tables pass → lane can close
    1  has required findings → needs repair
    2  blocked (no actionable findings, or structural issues)
    3  input error

Stdout JSON includes the gate decision and, if failing, the table IDs
...
def check_gate(review: dict) -> dict:
    ...
    if not tables:
        return {"gate": "blocked", "reason": "tables array is empty"}
    ...
    if status == "pass":
        for fig in tables:
            if fig.get("decision") != "pass":
                return {
                    "gate": "blocked",
                    "reason": f"top-level status is pass but {fig.get('table_id')} decision is {fig.get('decision')}",
                }
        return {"gate": "pass", "table_count": len(tables)}
    ...
    for fig in review.get("tables", []):
        if fig.get("decision") != "fail":
            continue
        failed_tables.append(fig.get("table_id"))
    ...
    if not failed_tables:
        return {"gate": "blocked", "reason": "status is fail but no failed tables"}

    if not required_findings:
        return {
            "gate": "blocked",
            "reason": "failed tables exist but no required findings — cannot build repair request",
