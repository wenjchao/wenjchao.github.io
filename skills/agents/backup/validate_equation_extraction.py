#!/usr/bin/env python3
"""Validate equation extractor worker output contract.
...
not judge visual quality or LaTeX correctness.
"""
...
VALID_RENDERERS = {"pdflatex", "matplotlib", "text_fallback"}
...
def validate_equation_identity(value: dict[str, Any], path: str, errors: list[str]) -> None:
    require_nonempty_string(value, "equation_id", path, errors)
    require_nonempty_string(value, "equation_number", path, errors)


def validate_candidates(data: Any, artifact_root: Path, errors: list[str]) -> None:
    root = require_object(data, "$.equation_candidates.json", errors)
    if root is None:
        return
    if root.get("schema_version") != SCHEMA:
        add(errors, "$.equation_candidates.json.schema_version", f'must be "{SCHEMA}"')
    require_nonempty_string(root, "worker_id", "$.equation_candidates.json", errors)

    pages = require_array(root.get("pages"), "$.equation_candidates.json.pages", errors)
    if pages is not None:
        for page_idx, page_obj in enumerate(pages):
            page_path = f"$.equation_candidates.json.pages[{page_idx}]"
    ...
    if not isinstance(root.get("unexpected_labeled_equations"), list):
        add(errors, "$.equation_candidates.json.unexpected_labeled_equations", "must be an array")


def validate_index(data: Any, errors: list[str]) -> None:
    root = require_object(data, "$.equation_index.json", errors)
    if root is None:
        return
    if root.get("schema_version") != SCHEMA:
        add(errors, "$.equation_index.json.schema_version", f'must be "{SCHEMA}"')
    require_nonempty_string(root, "worker_id", "$.equation_index.json", errors)
    equations = require_array(root.get("equations"), "$.equation_index.json.equations", errors)
    if equations is not None:
        seen: set[str] = set()
        for idx, equation in enumerate(equations):
            path = f"$.equation_index.json.equations[{idx}]"
            obj = require_object(equation, path, errors)
            if obj is None:
                continue
            validate_equation_identity(obj, path, errors)
            equation_id = obj.get("equation_id")
            if is_nonempty_string(equation_id):
                if equation_id in seen:
                    add(errors, f"{path}.equation_id", "must be unique")
    ...
    if not isinstance(root.get("omitted_candidates"), list):
        add(errors, "$.equation_index.json.omitted_candidates", "must be an array")
...
def validate_decisions(
    ...
    root = require_object(data, "$.equation_decisions.json", errors)
    if root is None:
        return
    if root.get("schema_version") != SCHEMA:
        add(errors, "$.equation_decisions.json.schema_version", f'must be "{SCHEMA}"')
    require_nonempty_string(root, "worker_id", "$.equation_decisions.json", errors)
    equations = require_array(root.get("equations"), "$.equation_decisions.json.equations", errors)
    if equations is not None:
        seen: set[str] = set()
        for idx, equation in enumerate(equations):
            path = f"$.equation_decisions.json.equations[{idx}]"
            obj = require_object(equation, path, errors)
            if obj is None:
                continue
            validate_equation_identity(obj, path, errors)
...
def validate_evidence_for_pass_equation(
    equation: dict[str, Any],
    path: str,
    errors: list[str],
) -> None:
    evidence = require_object(equation.get("evidence_read"), f"{path}.evidence_read", errors)
    if evidence is None:
        return
    ...
    rendered_latex_previews = evidence.get("rendered_latex_previews")
    ...
    if not isinstance(rendered_latex_previews, list):
        add(errors, f"{path}.evidence_read.rendered_latex_previews", "must be an array")
        rendered_latex_previews = []

    units = equation.get("crop_units", [])
    ...
    for unit_idx, unit in enumerate(units):
        ...
        if unit.get("preview") not in final_previews:
            add(errors, f"{path}.crop_units[{unit_idx}].preview", "pass equation must list preview in evidence_read")
        if unit.get("boundary_preview") not in boundary_previews:
            add(
                errors,
                f"{path}.crop_units[{unit_idx}].boundary_preview",
                "pass equation must list boundary_preview in evidence_read",
            )
            ...
            for bp in unit_bottom_band:
                if bp not in bottom_band_previews:
                    add(
                        ...
                        "pass equation must list bottom_band preview in evidence_read",
                    )
            ...
            for bm in unit_bottom_micro:
                if bm not in bottom_micro_previews:
                    add(
                        ...
                        "pass equation must list bottom_micro preview in evidence_read",
                    )
...
def validate_equations(
    ...
    root = require_object(data, "$.equations.json", errors)
    if root is None:
        return
    if root.get("schema_version") != SCHEMA:
        add(errors, "$.equations.json.schema_version", f'must be "{SCHEMA}"')
    require_nonempty_string(root, "worker_id", "$.equations.json", errors)
    if root.get("status") not in STATUS:
        add(errors, "$.equations.json.status", 'must be "complete" or "incomplete"')

    equations = require_array(root.get("equations"), "$.equations.json.equations", errors)
    verification_results: list[str] = []
    if equations is not None:
        seen: set[str] = set()
        numbers: set[str] = set()
        for idx, equation in enumerate(equations):
            path = f"$.equations.json.equations[{idx}]"
            obj = require_object(equation, path, errors)
            if obj is None:
                continue
            validate_equation_identity(obj, path, errors)
            ...
            if is_nonempty_string(equation_id):
                if equation_id in seen:
                    add(errors, f"{path}.equation_id", "must be unique")
                seen.add(equation_id)
            eq_num = obj.get("equation_number")
            if is_nonempty_string(eq_num):
                if eq_num in numbers:
                    add(errors, f"{path}.equation_number", f"duplicate equation_number: {eq_num}")
                numbers.add(eq_num)

            latex = obj.get("latex")
            if not is_nonempty_string(latex):
                add(errors, f"{path}.latex", "must be a non-empty string")
            elif not braces_balanced(latex):
                add(errors, f"{path}.latex", "LaTeX braces are unbalanced")

            renderer = obj.get("rendered_latex_renderer")
            if renderer is not None and renderer not in VALID_RENDERERS:
                add(errors, f"{path}.rendered_latex_renderer", f"must be one of {sorted(VALID_RENDERERS)}")

            rendered_preview = obj.get("rendered_latex_preview")
            if is_nonempty_string(rendered_preview):
                validate_relative_artifact_path(
                    rendered_preview, f"{path}.rendered_latex_preview", artifact_root, errors
                )

            uncertainties = obj.get("uncertainties")
            if not isinstance(uncertainties, list):
                add(errors, f"{path}.uncertainties", "must be an array")
            ...
            if verification is not None:
                ...
                for key in ("source_context_checked", "final_crop_checked",
                            "boundary_preview_checked", "latex_checked"):
                ...
                if "latex_checked" not in verification:
                    add(errors, f"{path}.verification.latex_checked", "is required for equation extraction")
            if result == "pass":
                validate_evidence_for_pass_equation(obj, path, errors)
                if isinstance(uncertainties, list) and len(uncertainties) > 0:
                    add(errors, f"{path}.uncertainties", "must be empty for a passing equation")
    ...
    if root.get("status") in STATUS and root["status"] != expected_status:
        add(errors, "$.equations.json.status", f"must be {expected_status} from verification results")
...
def validate_extraction(artifact_root: Path, paper_dir: Path | None) -> list[str]:
    ...
    required_files = {
        "equation_candidates.json": artifact_root / "equation_candidates.json",
        "equation_index.json": artifact_root / "equation_index.json",
        "equation_decisions.json": artifact_root / "equation_decisions.json",
        "equations.json": artifact_root / "equations.json",
    }
    ...
    if loaded["equation_candidates.json"] is not None:
        validate_candidates(loaded["equation_candidates.json"], artifact_root, errors)
    if loaded["equation_index.json"] is not None:
        validate_index(loaded["equation_index.json"], errors)
    if loaded["equation_decisions.json"] is not None:
        validate_decisions(
            loaded["equation_decisions.json"],
            artifact_root,
            paper_dir,
            errors,
        )
    if loaded["equations.json"] is not None:
        validate_equations(
            loaded["equations.json"],
            artifact_root,
            paper_dir,
            errors,
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate equation extractor worker output.")
