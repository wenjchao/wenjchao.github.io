...
valid figure_review.v3 that downstream scripts (check_review_gate.py,
...
Exit codes:
    0  success
    1  integrity error (duplicate figure_id, round mismatch)
    2  input error
"""
...
def main() -> int:
    ...
    for review_path in args.reviews:
        ...
        if review.get("schema_version") != "figure_review.v3":
            errors.append(f"{review_path}: schema_version is not figure_review.v3")
        ...
        for fig in review.get("figures", []):
            ...
            if figure_id in seen_figure_ids:
                errors.append(f"duplicate figure_id '{figure_id}' across input reviews")
    ...
    all_figures.sort(key=lambda f: f.get("figure_id", ""))

    pass_count = sum(1 for f in all_figures if f.get("decision") == "pass")
    fail_count = sum(1 for f in all_figures if f.get("decision") == "fail")
    finding_count = sum(len(f.get("findings", [])) for f in all_figures)
