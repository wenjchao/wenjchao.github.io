...
ALLOWED_TARGET_FILES = {"figures.json"}
...
def apply_manifest_patch(
    ...
    fig = next((f for f in data.get("figures", [])
                if f.get("figure_id") == fig_id), None)
    if fig is None:
        return f"figure_id {fig_id} not found"
