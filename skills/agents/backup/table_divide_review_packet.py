...
Each sub-packet gets a disjoint, consecutive subset of tables and is a valid
table_review_packet.v1.  The assignment manifest is printed to stdout.

Usage:
    python divide_review_packet.py <review_packet.json> \
        --workers 3 \
        --output-dir <paper_dir>/tables/reviewers/round_00
...
def divide(tables: list[dict], n: int) -> list[list[dict]]:
    per_worker = math.ceil(len(tables) / n)
    return [tables[i : i + per_worker] for i in range(0, len(tables), per_worker)]
...
def main() -> int:
    ...
    if not tables:
        print(json.dumps({"status": "error", "reason": "no tables in packet"}, indent=2),
              file=sys.stderr)
        return 2

    workers = min(args.workers, len(tables))
    if workers < args.workers:
        print(f"Warning: reduced workers from {args.workers} to {workers} "
              f"(only {len(tables)} tables)", file=sys.stderr)

    groups = divide(tables, workers)
    ...
    for idx, group in enumerate(groups):
        ...
        sub_packet = {
            "schema_version": packet.get("schema_version", "table_review_packet.v1"),
        ...
        assignments.append({
            "reviewer_id": reviewer_id,
            "table_ids": [fig.get("table_id") for fig in group],
