...
Exit codes:
    0  all figures pass → lane can close
    1  has required findings → needs repair
    2  blocked (no actionable findings, or structural issues)
    3  input error

Stdout JSON includes the gate decision and, if failing, the figure IDs
...
def check_gate(review: dict) -> dict:
    ...
    if not figures:
        return {"gate": "blocked", "reason": "figures array is empty"}
    ...
    if status == "pass":
        for fig in figures:
            if fig.get("decision") != "pass":
                return {
                    "gate": "blocked",
                    "reason": f"top-level status is pass but {fig.get('figure_id')} decision is {fig.get('decision')}",
                }
        return {"gate": "pass", "figure_count": len(figures)}
    ...
    for fig in review.get("figures", []):
        if fig.get("decision") != "fail":
            continue
        failed_figures.append(fig.get("figure_id"))
    ...
    if not failed_figures:
        return {"gate": "blocked", "reason": "status is fail but no failed figures"}

    if not required_findings:
        return {
            "gate": "blocked",
            "reason": "failed figures exist but no required findings — cannot build repair request",
