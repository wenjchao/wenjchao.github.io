...
- 輸出：
    ...
    - `figure_decisions.json`：在最終裁切前寫出的裁切決策（planning trace），記錄每張圖的裁切框、輸出檔、排除項目與決策理由。此檔不進 canonical，只留在 worker_output/ 作為決策紀錄。視覺驗證後修改 crop 時不需要回來同步此檔。
...
8. 在執行初次裁切之前，撰寫 `figure_decisions.json`。此檔案記錄初始裁切計劃和決策理由（planning trace），所有 `crop_px` 都必須是完整解析度頁面圖片的 pixel coordinate。視覺驗證後若需調整 crop，最終 `crop_px` 寫入 `figures.json`，不需回來同步此檔。每張圖應記錄：
   - figure label
...
9–11. 使用 `crop_and_preview.py` 一次完成裁切和全套 evidence preview：
...
12. 讀取每個 crop unit 的 evidence（每種可能有多個 `_seg<N>` 檔案，全部都要讀）：
...
17. 寫出/更新 `figures.json` 草稿後、回報成功前，執行 mandatory JSON self-validation（見規則段「機械自檢」）。validator 失敗時，必須修正 artifact 後重跑；validator 通過不代表視覺驗證通過。
...
- 不要在 artifact path 裡寫 `lanes/figures/worker_output/worker_01/`、`lanes/figures/canonical/` 或絕對路徑。
