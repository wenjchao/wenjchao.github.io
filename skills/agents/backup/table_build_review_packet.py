#!/usr/bin/env python3
"""Build review_packet.json from canonical tables.json.

Usage:
    python build_review_packet.py \
        <canonical_tables_json> \
...
def build_review_packet(
    ...
    packet = {
        "schema_version": "table_review_packet.v1",
        "review_round": review_round,
        "reviewer_id": reviewer_id,
        "canonical_artifact_root": "tables/canonical",
        "tables": [],
    }

    warnings = []

    for fig in tables_data.get("tables", []):
        table_id = fig.get("table_id", "")
        pf = {
            "table_id": table_id,
            "table_label": fig.get("table_label", table_id),
            "structured_json": fig.get("structured_json", ""),
            "rendered_table_preview": fig.get("rendered_table_preview", ""),
            "crop_units": [],
        }
        for cu in fig.get("crop_units", []):
            ...
            pcu = {
                ...
                "role": cu.get("role", "complete table"),
                "page": page,
            ...
            if canonical_root:
                for field in ("crop_preview", "boundary_preview"):
                    ...
                    if path and not (canonical_root / path).exists():
                        warnings.append(f"{table_id}/{cu.get('crop_id')}.{field}: "
                                        f"missing {path}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build review_packet.json from canonical tables.json"
    )
    parser.add_argument("tables_json", help="Path to canonical tables.json")
    ...
    canonical_root = None
    if args.paper_dir:
        canonical_root = Path(args.paper_dir) / "tables" / "canonical"
    ...
    if args.output:
        ...
        count = len(packet["tables"])
        print(f"Wrote {args.output} ({count} tables)", file=sys.stderr)
    else:
        print(output_text)
